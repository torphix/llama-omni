...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.lm_heads = nn.ModuleList(
            [
                ...
                for _ in range(config.output_heads)
            ]
        )
    ...
    def forward(self, text_idx=None, audio_idx=None, targets=None):
        ...
        if targets is not None:
            ...
            logits = torch.cat(
                [self.lm_head(x) for self.lm_head in self.lm_heads], dim=1
            )
            ...
            logits = torch.cat(
                [self.lm_head(x[:, [-1], :]) for self.lm_head in self.lm_heads], dim=1
            )
            loss = None
    ...
    @classmethod
    def from_pretrained(cls, model_type, override_args=None, config_kwargs=None):
        ...
        config_args = {
            "gpt2": dict(
                n_layer=12, n_head=12, n_embd=768, **config_kwargs
            ),  # 124M params
            "gpt2-medium": dict(
                n_layer=24, n_head=16, n_embd=1024, **config_kwargs
            ),  # 350M params
            "gpt2-large": dict(
